...
from .forms import RegistrationForm, LoginForm, InquiryForm
...
def add_listing(request):
    ...
    if request.method == 'POST':
        ...
        messages.success(request, 'Listing added successfully and is pending approval')
        return redirect('seller_dashboard')
...
def approve_listings_view(request):
    listings = Listing.objects.filter(is_approved=False)
    return render(request, 'admin/approve_listings.html', {'listings': listings})
...
def approve_listing(request, listing_id):
    ...
    return redirect('approve_listings')
